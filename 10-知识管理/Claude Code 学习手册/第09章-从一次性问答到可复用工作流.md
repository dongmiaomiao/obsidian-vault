...
本章让读者把“会问问题”升级为“有流程”。通过模板化提示词和固定输出格式，减少每次从头沟通的成本。重点是建立个人工作流文档，为后面的 CLAUDE.md、命令和 Skill 做基础。
...
常见任务模板 的重点不是记住术语，而是把它放回真实工程动作里理解。对于 Claude Code 来说，一个好的任务必须同时说明目标、边界、可用资料、允许执行的动作和验收方式。
...
输出格式控制 的重点不是记住术语，而是把它放回真实工程动作里理解。对于 Claude Code 来说，一个好的任务必须同时说明目标、边界、可用资料、允许执行的动作和验收方式。
...
验证优先 的重点不是记住术语，而是把它放回真实工程动作里理解。对于 Claude Code 来说，一个好的任务必须同时说明目标、边界、可用资料、允许执行的动作和验收方式。
...
建立个人工作流 的重点不是记住术语，而是把它放回真实工程动作里理解。对于 Claude Code 来说，一个好的任务必须同时说明目标、边界、可用资料、允许执行的动作和验收方式。
...
下一章衔接下一章将进入“Markdown 不是文档格式，而是 Agent 控制层”，继续把本章方法扩展到新的 Claude Code 使用场景。
