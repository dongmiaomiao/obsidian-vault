---
title: "第 30 章：Skill、Agent、MCP、Plugin 的组合模式"
type: chapter
...
本章用决策树方式讲“什么时候用什么”。如果只是一次性任务，用提示词；如果是固定流程，用命令；如果是可复用知识，用 Skill；如果需要专职角色，用 SubAgent；如果需要外部工具，用MCP；如果要分发给团队，用 Plugin。
...
组合选择 的重点不是记住术语，而是把它放回真实工程动作里理解。对于 Claude Code 来说，一个好的任务必须同时说明目标、边界、可用资料、允许执行的动作和验收方式。
...
Skill + SubAgent 的重点不是记住术语，而是把它放回真实工程动作里理解。对于 Claude Code 来说，一个好的任务必须同时说明目标、边界、可用资料、允许执行的动作和验收方式。
...
MCP + Plugin 的重点不是记住术语，而是把它放回真实工程动作里理解。对于 Claude Code 来说，一个好的任务必须同时说明目标、边界、可用资料、允许执行的动作和验收方式。
...
组合反模式 的重点不是记住术语，而是把它放回真实工程动作里理解。对于 Claude Code 来说，一个好的任务必须同时说明目标、边界、可用资料、允许执行的动作和验收方式。
...
下一章衔接下一章将进入“扩展系统实战小项目”，继续把本章方法扩展到新的 Claude Code 使用场景。
